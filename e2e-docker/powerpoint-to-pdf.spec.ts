...
test.describe("PowerPoint to PDF (real LibreOffice backend)", () => {
  test("converts a real .pptx to a real PDF via LibreOffice", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "PowerPoint to PDF" })).toBeVisible();
    ...
    await expect(page.getByRole("heading", { name: "Your presentation is now a PDF" })).toBeVisible({ timeout: 30_000 });
