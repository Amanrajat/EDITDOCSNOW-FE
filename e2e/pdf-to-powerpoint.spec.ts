...
test.describe("PDF to PowerPoint", () => {
  test("creates one slide with real text and a real table", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "PDF to PowerPoint" })).toBeVisible();
    ...
    await expect(page.getByRole("heading", { name: "Your PDF is now a slide deck" })).toBeVisible({ timeout: 15_000 });
